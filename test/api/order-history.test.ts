...
void describe('/rest/order-history', () => {
  void it('GET own previous orders', async () => {
    ...
    assert.equal(res.body.data[0].products[0].name, 'Canadian Equity Index Fund (CDEQ)')
    ...
    assert.equal(res.body.data[0].products[1].name, 'US Total Market Index Fund (USTM)')
    ...
    assert.equal(res.body.data[1].products[0].name, 'Global Equity Growth Fund (GLBL)')
