...
void describe('/rest/user/data-export', () => {
  ...
  void it('Export data including orders without use of CAPTCHA', async () => {
    ...
    assert.equal(parsedData.orders[0].products[0].name, 'Emerging Markets Equity Fund (EMEQ)')
  ...
  void it('Export data including reviews without use of CAPTCHA', async () => {
    ...
    assert.equal(parsedData.reviews[0].message, 'Bought a small position for my TFSA. Liquidity is thin, so expect wider spreads.')
    ...
    assert.equal(parsedData.reviews[1].message, 'Small allocation, but the underlying holdings are genuinely new-economy names.')
